...
function UserRankingCard(p: {
    ...
    return (
        <tr class={`${!p.row.user.is_active ? 'opacity-75 bg-base-300' : ''} hover:rounded-lg hover:bg-base-300 `}>
            ...
            <td>
                <div class="flex flex-row items-center gap-2">
                    ...
                    <Link url={`/users/${p.row.user.id}`} css="underline-1 p-1 hover:underline">
                        {p.row.user.username}
                    </Link>
                </div>
            </td>
