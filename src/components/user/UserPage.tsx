...
async function UserPage(p: { logged: UserCookie | null; user_id: string; mode?: Mode; lang: string }) {
    ...
    const panels: PanelType[] = [
        {
            ...
            jsx: <UserHistoryPanel db_ranks={user.db_ranks} play_counts={user.monthly_playcounts} replays_watched={user.replays_watched_counts} />,
        },
        {
    ...
    return (
        <>
            ...
            <nav class="sticky top-16 z-40 -mt-8 flex flex-row flex-wrap items-center justify-center gap-x-6 gap-y-2 rounded-b-lg bg-base-300 p-2 text-sm text-neutral-content underline-offset-1 shadow-lg">
                <a class="hover:underline" href="#top">
                    {txt(p.lang, "user.sections.top")}
                </a>
                {panels.map((p) => {
                    if (!p.show_if) return <></>;
                    return (
                        <a class="hover:underline" href={`#${p.code}`}>
                            {p.title}
                        </a>
                    );
                })}
