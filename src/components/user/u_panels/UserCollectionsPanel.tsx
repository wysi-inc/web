...
async function UserCollectionsPanel({ user_id, logged_id, collections }: Props) {
    ...
    return (
        <div id="colpanel" class="flex flex-col gap-4">
            {editable ? (
                <form
                    id="collections_form"
                    ...
                    hx-post={`/users/${user_id}/collections/parse`}
                >
                    <fieldset id="collections_fieldset" class="group join" disabled>
                        <input
                            ...
                            name="collection"
                            required
                            class="peer file-input join-item file-input-bordered file-input-sm w-full max-w-xs group-disabled:hidden"
                        />
                    ...
                    <div class="flex flex-row gap-2">
                        ...
                        <button
                            type="button"
                            class="btn btn-sm hidden flex-row items-center gap-2"
                            id="collections_form_delete"
                            onclick="collections_delete_modal.showModal()"
                        >
                            <i class="fa-regular fa-trash-can" />
                            <span>Delete ALL</span>
                        </button>
                        <button type="reset" class="btn btn-error btn-sm hidden" id="collections_form_cancel">
                        ...
                        <dialog id="collections_delete_modal" class="modal">
                            <div class="modal-box">
                                ...
                                <div class="modal-action">
                                    <button class="btn btn-error" hx-swap="innerHTML" hx-target="#colpanel" hx-delete={`/users/${user_id}/collections/delete`}>
                                        Yes, delete them
                                    </button>
                                    <form method="dialog">
            ...
            {collections && collections.length > 0 ? (
                <>
                    <div class="flex flex-row flex-wrap gap-2">
                        <button
                            class="collection-download-button btn btn-info btn-sm flex h-8 grow cursor-pointer flex-row items-center gap-2"
                            id={`btn_download_${user_id}`}
                            data-name={`all_collections_${user_id}`}
                            data-ids={JSON.stringify(collections.map((c) => c.beatmapsMd5.map((h) => h)).flat())}
                        >
                            <i class="fa-regular fa-file-zipper" />
                            <span class="loading loading-spinner loading-xs" style={{ display: "none" }} />
                            ...
                            <progress
                                ...
                                value={0}
                                max={collections.map((c) => c.beatmapsMd5.length).reduce((total, c) => total + c, 0)}
                                style={{ display: "none" }}
                            />
                            ...
                            <div class="flex flex-col gap-2 rounded-lg bg-base-300 p-2">
                                <details class="group">
                                    <summary class="flex cursor-pointer flex-row items-center justify-between gap-4 rounded-lg bg-neutral ps-4">
                                        ...
                                        <button
                                            class="collection-download-button btn btn-info btn-sm flex h-8 cursor-pointer flex-row items-center gap-2"
                                            id={`btn_download_${c.name}`}
                                            data-name={c.name}
                                        ...
                                        >
                                            ...
                                            <progress class="progress progress-success w-56" value={0} max={c.beatmapsMd5.length} style={{ display: "none" }} />
                                            <span class="progress-indicator" style={{ display: "none" }}>
            ...
            <Title scripts={["/public/js/collections.js"]} modules={["/public/js/collectiondownloader.js"]} />
        </div>
    );
}
