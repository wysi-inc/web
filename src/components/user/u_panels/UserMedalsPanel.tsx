...
async function UserMedalsPanel(p: { user_id: number; medals: ProfileMedal[]; lang: string }) {
    ...
    medals
        .sort((m) => m.Ordering - m.Ordering)
        .forEach((m) => {
            if (!categories[m.Grouping]) {
                categories[m.Grouping] = { count: 0, medals: [] };
            }
            if (m.achieved) {
                categories[m.Grouping].count++;
            }
            categories[m.Grouping].medals.push(m);
        });

    const recent_medals = medals
        .filter((m) => m.achieved)
        .sort((a, b) => {
        ...
        <>
            <div class="flex flex-row flex-wrap gap-4">
                <div class="grow rounded-lg bg-neutral shadow-lg">
                    <div class="p-1 px-2 text-neutral-content">Recent Medals</div>
                    <div class="flex flex-row flex-wrap gap-2 rounded-lg bg-base-300 p-4">
                        {recent_medals.length > 0 ? recent_medals.map((m) => <MedalBadge medal={m} />) : <div>No recent medals</div>}
                    </div>
                </div>
                <div class="grow rounded-lg bg-neutral shadow-lg">
                    <div class="p-1 px-2 text-neutral-content">Rarest Medal ({rarest_medal?.Frequency?.toFixed(2)}%)</div>
                    <div class="flex items-center justify-center rounded-lg bg-base-300 p-4">
                        {rarest_medal ? <MedalBadge medal={rarest_medal} /> : <span>???</span>}
                    </div>
                </div>
            </div>
