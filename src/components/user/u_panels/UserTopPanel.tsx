...
async function UserTopPanel(p: { lang: string; user: UserExtended; mode: Mode; editable?: boolean }) {
    ...
    let has_info = false;
    if (p.user.location) has_info = true;
    else if (p.user.interests) has_info = true;
    else if (p.user.occupation) has_info = true;
    else if (p.user.website) has_info = true;
                        ...
                        <div class="flex flex-row flex-wrap gap-4">
                            <div class="flex w-40 flex-col gap-4">
                                ...
                                <span class="text-center">
                                    joined{" "}
                                    <time class="tooltip" data-tip={joined_date} datetime={joined_date}>
                                        {moment(p.user.join_date).fromNow()}
                                    </time>
                                </span>
                            </div>
                            <div class="flex grow flex-col items-start justify-between gap-2">
                                {(p.user.wysi_badges?.length || 0) > 0 ? (
                                    <div class="flex flex-row flex-wrap gap-2">
                                        {p.user.wysi_badges?.map((b) => <Badge user_id={p.user.id} badge_id={b} editable={false} />)}
                                    </div>
                                ) : null}
                                <div class="flex flex-row items-center gap-2">
                                    ...
                                    {p.user.is_supporter && <Supporter level={p.user.support_level} />}
                                    {p.user.groups.map((g) => (
                                        <div class="badge flex flex-row border-none p-1 text-white" style={{ backgroundColor: g.colour, gap: ".08rem" }}>
                                            {g.short_name}
                                        </div>
                                    ))}
                                </div>
                                {p.user.title ? (
                                    <div class="inline-block bg-gradient-to-r from-blue-600 to-green-400 bg-clip-text text-transparent">{p.user.title}</div>
                                ) : null}
                                <div class="flex flex-row items-center gap-2">
                                ...
                                <dl class="flex flex-col gap-1">
                                    <div>
                                        ...
                                        <dd class="text-lg">
                                            {Math.round(p.user.statistics.pp).toLocaleString()}
                                            pp
                                        </dd>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <dd class="text-lg">
                                            {p.user.user_achievements.length} <i class="fa-solid fa-medal fa-xs"></i>
                                        </dd>
                                    </div>
                                    {p.mode === "mania" ? (
                                        p.editable ? (
                                ...
                                <table>
                                    ...
                                    <tr>
                                        ...
                                        <td class="p-1 text-end">
                                            {Math.round(p.user.statistics.total_hits / p.user.statistics.play_count || 0).toLocaleString()}
                                        </td>
                                    </tr>
                                    <tr>
                                        <th class="p-1">
                    ...
                    <div class="flex flex-col gap-4 p-4">
                        {has_socials || p.editable ? (
                            <>
                                <div class="flex flex-row flex-wrap gap-2">
                                    <form
                                        class="sortable flex flex-row flex-wrap items-center gap-2"
                                        id="socials_list"
                                        hx-post={`/users/${p.user.id}/socials/sort`}
                                        hx-trigger="end"
                                        hx-swap="none"
                                    >
                                    ...
                                    {p.editable ? (
                                        <form
                                            class="group flex flex-row flex-wrap items-center gap-2"
                                            hx-put={`/users/${p.user.id}/socials/submit`}
                                            hx-target="#socials_list"
                                            hx-swap="beforeend"
                                        >
                                            <span class="htmx-indicator loading loading-spinner" />
                                            ...
                                            <button
                                                class="btn btn-square btn-ghost btn-sm peer-enabled:hidden"
                                                onclick="document.querySelector('#socials_fieldset').disabled = false"
                                                type="button"
                                            >
                                                <i class="fa-solid fa-plus" />
                                            </button>
                                            <button
                                                class="btn btn-square btn-ghost btn-sm peer-disabled:hidden"
                                                onclick="document.querySelector('#socials_fieldset').disabled = true"
                                                type="button"
                                            >
                            ...
                            <div class="flex flex-row flex-wrap gap-4 text-sm">
                                ...
                                {p.user.website ? (
                                    <div class="flex flex-row items-center gap-2">
                                        ...
                                        <a href={p.user.website} target="_blank" class="hover:underline">
                                            {p.user.website}
                                        </a>
                                    </div>
                                ) : null}
