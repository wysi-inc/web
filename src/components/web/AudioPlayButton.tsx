const AudioPlayButton = (p: { set_id: number; map_id: number; set_title: string; set_artist: string; join?: boolean }) => (
    <>
        <button
            class={`group/audio ${p.join ? "join-item btn-info" : "no-animation size-full"} btn hidden border-none group-hover/audio_card:flex group-hover/audio_card:bg-opacity-75 data-[active]:flex data-[active]:bg-opacity-75`}
