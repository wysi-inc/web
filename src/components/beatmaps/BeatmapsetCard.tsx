...
const BeatmapsetCard = (props: Props) => {
    ...
    return (
        <div class="rounded-lg flex flex-row bg-neutral shadow-lg">
            <div class="bg-base-100 flex flex-col grow rounded-lg shadow-lg">
                <div class="rounded-lg overflow-hidden grow flex flex-col shadow-lg" style={`background-image: url(${coverImg}); background-size: cover;`}>
                    <div class="flex gap-4 p-4 flex-row grow" style="backdrop-filter: blur(10px); background-color: rgba(0, 0, 0, 0.6);">
                        <img src={listImg} alt="cover" class="rounded-lg" style="heigth: 80px; width: 60px; object-fit: cover;" />
                        <div class="flex flex-col justify-between grow">
                            <div class="truncate w-72 text-lg">{beatmapset.title}</div>
                            <div class="truncate w-72 text-sm">by {beatmapset.artist}</div>
                            <div class="truncate w-72 text-sm">{beatmapset.creator}</div>
                        </div>
                    </div>
                </div>
