...
export async function getBeatmaps(q: BeatmapQuery): Promise<Beatmapset[]> {
    const res: any = await v2.beatmaps.search({
        query: [
            q.title && q.title,
        ...
        sort: "updated_desc",
        section: q.status === "all" ? undefined : q.status as any,
        mode: q.mode === "all" ? undefined : q.status as any,
    });
    if (res.error) {
        return [];
    }
    return res.beatmapsets;
}
